...
def research_download(file_url, local_file_path, data_row, header):
    try:
        ...
        if response.status_code == 200:
            ...
            cos_tool.upload_file(local_file_path, local_file_path)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                cos_tool = get_origin_file_cos_instance()
                cos_tool.upload_file(local_file_path, local_file_path)
                print(f'File successfully uploaded to Tencent Cloud: {local_file_path}')
            except Exception as e:
                print(f'Error uploading file to Tencent Cloud: {str(e)}')
            print(f'file download success {file_url}')
            return True
        else:
            add_error_log(message=f"file download fail code:{response.status_code}")
